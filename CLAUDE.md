...
- **Gated tools** (`DefaultPolicy: tools.PolicyGate`) must have the tool handler as the **only** entry point to the underlying dangerous operation. Either make the operation package-private, or guard it on a ctx marker set only by the registry path. No callers from other packages that reach around the tool, no MCP handlers that bypass `tools.Registry.Execute`. See `.claude/skills/gated-tools-guide.md`.
- **Substantial system-prompt prose lives in `prompts/*.tmpl` files.** If you're writing more than a sentence or two of prose that ships to the LLM as system prompt or system suffix (e.g. an app's `SystemPrompt()`, a card-chat suffix, a job prompt), put it in a `.tmpl` file under a `prompts/` directory adjacent to its caller. Name files by role: `system_*.tmpl` for content that ends up in a system block, `user_*.tmpl` for user-message bodies — the prefix lets a reader tell at a glance what the prompt is for. Embed with `//go:embed prompts/*.tmpl`, parse via `internal/prompts.MustParse` at package scope, render with `prompts.Render(set, "name.tmpl", data)`. Use `text/template` (`{{ .Field }}`); never `html/template` (it HTML-escapes). One-line `fmt.Sprintf` helpers next to their data, tool descriptions in `ToolMetas()`, and short user-message bodies are fine to keep inline — the goal is to get the big prose blocks out of `*.go` source, not to template every string.
- **Never write a future date into a test fixture.** A literal like `StartsAt: "2026-09-19 00:00"` is a time bomb: it passes until that day, then the event falls behind a feed/window filter and the test fails with an empty document — so the message reads "DTSTART is not a DATE value" over a calendar that has no DTSTART in it, and points away from its cause. Build dates relative to now (`time.Now().AddDate(0, 0, 7)`, or the package's `dayOut`/`defaultFixtureStart` helper) and **derive the assertion from the same value the fixture wrote**, never from a second literal you have to keep in step by hand. Past-dated literals are fine — the past stays past — and so is a literal under a frozen clock (`timeNow = func() ...`), which is the right tool when the test is *about* a specific calendar date, like a DST boundary. When comparing against a date the database computes (`CURRENT_DATE`), take today from the database, not the runner's clock: they disagree for six hours a day. See `15f47d9`.
