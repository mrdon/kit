...
ALTER TABLE app_card_scopes ADD COLUMN scope_id UUID REFERENCES scopes(id) ON DELETE CASCADE;

UPDATE app_card_scopes cs
SET scope_id = s.id
FROM scopes s
...
DECLARE
    null_count INT;
BEGIN
    SELECT COUNT(*) INTO null_count FROM app_card_scopes WHERE scope_id IS NULL;
    IF null_count > 0 THEN
        RAISE EXCEPTION 'Migration failed: % app_card_scopes rows have NULL scope_id (orphan role/user references)', null_count;
    END IF;
END $$;

ALTER TABLE app_card_scopes ALTER COLUMN scope_id SET NOT NULL;

DROP INDEX IF EXISTS idx_app_card_scopes_lookup;
ALTER TABLE app_card_scopes DROP CONSTRAINT app_card_scopes_pkey;
ALTER TABLE app_card_scopes ADD PRIMARY KEY (card_id, scope_id);
ALTER TABLE app_card_scopes DROP COLUMN scope_type;
ALTER TABLE app_card_scopes DROP COLUMN scope_value;

CREATE INDEX idx_app_card_scopes_lookup ON app_card_scopes(tenant_id, scope_id);

-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
ALTER TABLE app_card_scopes ADD COLUMN scope_type TEXT;
ALTER TABLE app_card_scopes ADD COLUMN scope_value TEXT;
...
ALTER TABLE app_card_scopes ALTER COLUMN scope_type SET NOT NULL;
ALTER TABLE app_card_scopes ALTER COLUMN scope_value SET NOT NULL;
DROP INDEX IF EXISTS idx_app_card_scopes_lookup;
ALTER TABLE app_card_scopes DROP CONSTRAINT app_card_scopes_pkey;
ALTER TABLE app_card_scopes ADD PRIMARY KEY (card_id, scope_type, scope_value);
ALTER TABLE app_card_scopes DROP COLUMN scope_id;
