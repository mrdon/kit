...
BEGIN
    WITH deleted AS (
        DELETE FROM app_calendar_scopes cs
    ...
    IF n > 0 THEN
        RAISE NOTICE 'app_calendar_scopes: deleted % orphan scope rows', n;
    END IF;
END $$;
...
ALTER TABLE app_calendar_scopes ADD COLUMN scope_id UUID REFERENCES scopes(id) ON DELETE CASCADE;

UPDATE app_calendar_scopes cs
SET scope_id = s.id
FROM scopes s
...
DECLARE
    null_count INT;
BEGIN
    SELECT COUNT(*) INTO null_count FROM app_calendar_scopes WHERE scope_id IS NULL;
    IF null_count > 0 THEN
        RAISE EXCEPTION 'Migration failed: % app_calendar_scopes rows have NULL scope_id', null_count;
    END IF;
END $$;

ALTER TABLE app_calendar_scopes ALTER COLUMN scope_id SET NOT NULL;

ALTER TABLE app_calendar_scopes DROP CONSTRAINT app_calendar_scopes_pkey;
ALTER TABLE app_calendar_scopes ADD PRIMARY KEY (tenant_id, calendar_id, scope_id);
ALTER TABLE app_calendar_scopes DROP COLUMN scope_type;
ALTER TABLE app_calendar_scopes DROP COLUMN scope_value;

-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
ALTER TABLE app_calendar_scopes ADD COLUMN scope_type TEXT;
ALTER TABLE app_calendar_scopes ADD COLUMN scope_value TEXT;
...
ALTER TABLE app_calendar_scopes ALTER COLUMN scope_type SET NOT NULL;
ALTER TABLE app_calendar_scopes ALTER COLUMN scope_value SET NOT NULL;
ALTER TABLE app_calendar_scopes DROP CONSTRAINT app_calendar_scopes_pkey;
ALTER TABLE app_calendar_scopes ADD PRIMARY KEY (tenant_id, calendar_id, scope_type, scope_value);
ALTER TABLE app_calendar_scopes DROP COLUMN scope_id;
-- +goose StatementEnd
