...
// to the given user. If threadTS is non-empty, the ephemeral attaches
// to that thread so the user sees it in-context with the conversation;
// otherwise it appears at channel level. Used by side-channel paths
// that must not land the message in conversations.history (e.g. the
// vault deep-link tool, which posts a one-shot login URL the LLM must
// never see). Slack docs: https://api.slack.com/methods/chat.postEphemeral
