...
//   - JSON API routes under /{slug}/api/vault/... that the React console
//     vault (/{slug}/web/vault) posts to, plus a reveal bridge route at
//     /{slug}/apps/vault/reveal/... that turns a Slack one-shot token
//     into a console session (all authenticated via the session cookie).
