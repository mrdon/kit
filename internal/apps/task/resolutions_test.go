...
func TestSnoozeMonthThenMondayFromWednesday(t *testing.T) {
	loc, _ := time.LoadLocation("America/Los_Angeles")
	// Wed 2026-04-22 14:00 PDT → +30d = Fri 2026-05-22 → next Mon 2026-05-25 07:00 PDT.
	now := time.Date(2026, 4, 22, 14, 0, 0, 0, loc)
	got, err := snoozeMonthThenMondayAt(now, "America/Los_Angeles")
	if err != nil {
		t.Fatalf("snoozeMonthThenMondayAt: %v", err)
	}
	local := got.In(loc)
	if local.Weekday() != time.Monday {
		t.Errorf("expected Monday, got %s", local.Weekday())
...
func TestSnoozeMonthThenMondayWhen30dLandsOnMonday(t *testing.T) {
	loc, _ := time.LoadLocation("America/New_York")
	// Sat 2026-04-25 → +30d = Mon 2026-05-25; should keep that Monday, not skip a week.
	now := time.Date(2026, 4, 25, 9, 0, 0, 0, loc)
	got, err := snoozeMonthThenMondayAt(now, "America/New_York")
	if err != nil {
		t.Fatalf("snoozeMonthThenMondayAt: %v", err)
	}
	local := got.In(loc)
...
func TestSnoozeMonthThenMondayBadTimezone(t *testing.T) {
	if _, err := snoozeMonthThenMondayAt(time.Now(), "Not/A/Zone"); err == nil {
		t.Fatal("expected error for bad timezone")
	}
}
