...
treated as UTC).`,
		Schema: services.PropsReq(map[string]any{
			...
			"participants":     services.Field("array", "Array of Slack user IDs to DM (e.g. ['U09...', 'U07...']). Use find_user first. Don't include the organizer."),
			"duration_minutes": services.Field("integer", "Optional. Meeting length in minutes. Inferred from candidate_slots if omitted."),
			"start_date":       services.Field("string", "Optional (YYYY-MM-DD). Inferred from earliest candidate slot if omitted."),
			"end_date":         services.Field("string", "Optional (YYYY-MM-DD). Inferred from latest candidate slot if omitted."),
			"notes":            services.Field("string", "Optional preferences/context like 'mornings preferred'. Also accepted as 'description'."),
			"description":      services.Field("string", "Alias for notes."),
		...
		}, "title", "candidate_slots", "participants"),
	},
	{
