...
// wireAward is one honourable mention on the wire. Public on every surface:
// the whole point is that the room sees which table took what, and a phone
// showing only its own would leave the other nineteen with nothing.
...
func publicAwards(s *Snapshot) []wireAward {
	out := make([]wireAward, 0, len(s.Awards))
	for _, a := range s.Awards {
		out = append(out, wireAward{
			Key: a.Key, Title: a.Title, TeamID: a.TeamID.String(),
			TeamName: a.TeamName, Detail: a.Detail,
		})
	}
	return out
}
