	...
	return mcpauth.WithCaller(func(ctx context.Context, req mcp.CallToolRequest, caller *services.Caller) (*mcp.CallToolResult, error) {
		...
		if rerr != nil {
			...
			case errors.Is(rerr, ErrNetlifyNotConnected):
				return mcp.NewToolResultError("netlify not connected — visit /web/admin/integrations to connect"), nil
			case errors.Is(rerr, ErrGitHubNotConnected):
				return mcp.NewToolResultError("github not connected — visit /web/admin/integrations to install the Kit GitHub App"), nil
			}
