...
func newItemFixture(t *testing.T) *itemFixture {
	...
	user, err := models.GetOrCreateUser(ctx, pool, tenant.ID, "U_builder_"+uuid.NewString()[:8], "Builder User", true)
	if err != nil {
...
func TestItemService_DeleteOne_HistoryHasDeleterProvenance(t *testing.T) {
	...
	deleterRun := uuid.New()
	deleterUser, err := models.GetOrCreateUser(ctx, f.pool, f.tenant.ID, "U_deleter_"+uuid.NewString()[:8], "Deleter", true)
	if err != nil {
