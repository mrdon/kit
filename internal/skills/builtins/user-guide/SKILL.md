...
**A break before the final is optional, and off by default.** When the last board is spent the game waits on the empty board with **Start the final** ready, so you press once. On a longer night you may want the beat instead: **Settings → Break before the final** puts the standings up and holds the room until you start it. Off is the default because on a short night it stops everybody dead one question from the end, at the point they have finally stopped talking. You can always call a break yourself either way.
...
**The ending is two presses.** **End the night** puts the honorable mentions on the screen, and **Show the winner** brings up the podium. Nothing is on a clock in between, so you can read the mentions out at whatever pace the room is going at. Say something like "before we announce the winner, a few honorable mentions", press once, work down the list, then press again.

**What the mentions are.** Three to five side awards, one per table, from a pool of seventeen. Most measure how a table played rather than how well: **Backed themselves** and **Trusted the room** for where a table put its chips, **Wildest guesses** for the numbers furthest outside the rest of the room, **So close all night** for the table that held the nearest guess that was over and got paid nothing for it, **Doubted themselves** for writing the winning answer and betting elsewhere. No table takes two, so they find three to five different tables on most nights, and a table that finished tenth can still get its name on the wall.

The list is on your own panel in the order the screen shows it, so you are reading your laptop rather than the TV over your shoulder. A small room gets fewer of them rather than repeats, and a table that joined near the end is left out, because three questions is not a night. A night that earns none at all skips the screen entirely and goes straight to the podium.
