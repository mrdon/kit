---
name: builder-admin-guide
description: "How to build tenant-scoped apps with Kit's builder substrate: the create_app → app_create_script → expose/schedule workflow, identity via current_user() and per-user data scoping, admin-only edit patterns, service-layer composition, field-type conventions, validation, concurrency gotchas, LLM budget awareness, error handling, a worked CRM example, a 'from admin prompt to working app' recipe, and a full reference of meta-tools and Monty built-ins. Load whenever an admin asks to build, modify, debug, schedule, or expose a script-backed app — including follow-ups like 'add a field' or 'make it per-user'."
admin_only: true
---
